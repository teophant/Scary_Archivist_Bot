...
# ==== КОМАНДА /setup (только для администраторов группы) ====
...
def setup_group(message):
    ...
    try:
        ...
        bot.reply_to(message, "⚠️ Не удалось проверить права.")
        return
    
    # Создаём приветственное сообщение с кнопкой
    welcome_text = """
🎭 **СБОРНИК ГОРОДСКОГО ФОЛЬКЛОРА**

Знаете интересную городскую историю, легенду или байку? 
Видели что-то странное или необычное?
Помните старые городские истории?
...
Можно совместить всё в одной истории!
...
👇 Нажмите кнопку ниже, чтобы начать 👇
    ...
    try:
        ...
        # Пытаемся закрепить (если у бота есть права)
        try:
            bot.pin_chat_message(message.chat.id, sent_message.message_id, disable_notification=True)
            bot.reply_to(message, "✅ Готово! Сообщение создано и закреплено.")
        except:
            bot.reply_to(message, "✅ Сообщение создано! (Не удалось закрепить - дайте боту права администратора для закрепления)")
        
        # Удаляем команду /setup для чистоты
        try:
